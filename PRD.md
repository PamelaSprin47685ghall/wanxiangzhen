    ...
    for msg in msgs (按时间序):
        for fm in EventCodec.decodeEvents(msg.text):   // multi-frontmatter：一条消息可含多块
            match fm.squad_event:
              "tasks_created":  for t in fm.tasks:    // 一次拆解多个 task
                  dag.tasks.set(t.task_id, { ...t, status:"pending" })
              "task_started":  dag.tasks[fm.task_id].status = "running"
              "task_submitted":dag.tasks[fm.task_id].status = "submitted"
              "task_merged":   dag.tasks[fm.task_id].status = "merged"; dag.tasks[fm.task_id].mergedSha = fm.master_sha
              "task_done":     dag.tasks[fm.task_id].status = "done"
...
3. 将聚合后的全部 task 构造为单个 TasksCreated 事件（multi-frontmatter，附录 D）→ 经注入队列 session.prompt 写入历史；squad_cancelled 则调用 handleSquadKill 单独注入 squad_cancelled 事件
4. 触发 Scheduler.tick()
