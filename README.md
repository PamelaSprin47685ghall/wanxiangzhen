...
| **`.wanxiangzhen.ndjson`** | **SSOT**（项目根 NDJSON 事件流，每行含 `session`）。意图不落盘，事实 append；内存 DAG 为 fold 投影。启动/首次 master session 时 `replayFromEventLog` 重放；先 append 后改内存；文件锁串行写。详见 `PRD/EventSourcing.md`。 |
