...
let handleSquadUpdate (rt: CoordinatorRuntime) (args: obj) : JS.Promise<string> =
    promise {
        ...
        let eventsArrOpt : obj array option =
        ...
        match eventsArrOpt with
        | None -> return formatSquadUpdateOutcome (InvalidInput "events must be a non-empty array.")
        | Some arr when arr.Length = 0 -> return formatSquadUpdateOutcome (InvalidInput "events must be a non-empty array.")
        | Some rawEvents ->
            if rt.Dag.SessionId = "" then
                ...
                let rawEvents = Array.toList rawEvents
                        ...
                        (promise {
                            ...
                            if dangling <> [] then
                                ...
                                | None ->
                                    ...
                                    match appendOk with
                                    | Error err -> rt.InjectError <- Some (sprintf "TasksCreated append failed: %s" err); return "Error: event log append failed."
                                    | Ok () ->
