...
Shell（副作用边界）:
  ...
  SessionIo       promptSession + readAllTexts（session.prompt/messages SDK 包装）
  ...
  CoordinatorLifecycle  replayFromHistory + handleSquadKill + handleSquadUpdate + create/createWithDeps
...
6. 初始化空内存 DAG；首次拿到 master sessionID 后调 `replayFromHistory` 重放重建（§5.4）
7. 起 PID 健康轮询定时器（§5.9）
...
2. **捕获 master sessionID**：`masterSessionId ??= input.sessionID`（§5.4），触发 `replayFromHistory`。
...
3. replayFromHistory（§5.4：NDJSON + git 校正）
...
| 用户重启 coordinator | 插件加载 → `replayFromHistory` 读 `.wanxiangzhen.ndjson` fold + git 二次校正重建 DAG（§5.4）→ 启动新 HTTP server（新随机端口 + 新 token）| 决策 1.3 |
...
| 重放 | `inferReviewTaskFromTexts` 扫 frontmatter 标量 | `replayFromHistory` 读 `.wanxiangzhen.ndjson` fold + git 二次校正（§5.4 / `EventSourcing.md`）|
