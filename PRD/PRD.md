...
```
function replayFromEventLog(projectRoot):
    lines = readAndTruncateBadLines(join(projectRoot, ".wanxiangzhen.ndjson"))
    events = parseLinesToSquadEvents(lines)   // 全局行序
    dag = empty; sessions = {}
    for ev in events:
        match ev:
...
3. 构造 `TasksCreated` → **append** `.wanxiangzhen.ndjson`（成功后再更新内存）；`squad_cancelled` → `handleSquadKill` + append
4. 触发 Scheduler.tick()
...
spawnSlave(task):
  1. env = { ...process.env,
             ...
             SQUAD_TOKEN:          token }
   2. initialPrompt = Prompts.buildSlavePrompt(task)   // §6.2，固定按 /loop 可用构造
...
多源并发 append。所有 NDJSON 写在 **文件锁** 内串行；进程内 `SerialQueue` 排队 append Promise。

```
appendSquadEvent(ev):
  withFileLock(projectRoot):
    appendLine(".wanxiangzhen.ndjson", encodeLine(ev))
  on Ok: foldEvent into Dag (若调用方尚未 fold)
  optional: notifyLlm(encodeEvent(ev))  // 非 SSOT，失败可忽略
```
...
slave prompt 固定按 万象术 /loop（With-Review Mode）可用构造：

```
万象术 /loop（With-Review Mode）已启用。请按以下流程开发：
...
[2] coordinator LLM 分析需求 → 调用 squad_update（§5.6）:
     ...
     → squad_update：拓扑校验通过 → 更新内存 DAG → 注入 1 条 tasks_created 事件（multi-frontmatter） → Scheduler.tick()
...
- [ ] 重启重放：`.wanxiangzhen.ndjson` fold + git 二次校正（§5.4 / `EventSourcing.md`）
...
replayDagFromHistory(messages):
  ...
  for msg in messages:                                  # session.messages() 全量（核实 5.4）
    for fm in EventCodec.decodeEvents(msg.text):        # multi-frontmatter：一条消息可含多块
      match fm.squad_event:
        squad_created   → 记录 session + requirement
        tasks_created   → for t in fm.tasks:
                              dag[t.task_id] = { ...t, status: pending }
        task_started    → dag[fm.task_id].status = running
        task_submitted  → dag[fm.task_id].status = submitted
        task_merged     → dag[fm.task_id].status = merged
        task_done       → dag[fm.task_id].status = done
